...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (target.roles.cache.has(role.id)) return interaction.reply({ embeds: [{ color: 0xff8c00, description: `⚠️ <@${target.id}> already has <@&${role.id}>.` }], ephemeral: true });

    try {
      await target.roles.add(role);
      await interaction.reply({
        embeds: [{
          color: 0x00cc66, title: "✅ Role Given",

...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!target.roles.cache.has(role.id)) return interaction.reply({ embeds: [{ color: 0xff8c00, description: `⚠️ <@${target.id}> doesn't have <@&${role.id}>.` }], ephemeral: true });

    try {
      await target.roles.remove(role);
      await interaction.reply({
        embeds: [{
          color: 0xff8c00, title: "🔻 Role Removed",

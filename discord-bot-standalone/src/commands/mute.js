...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((o) =>
      o.setName("member").setDescription("Member to mute").setRequired(true)
    )
    .addStringOption((o) =>
      o.setName("duration")
        .setDescription("How long to mute them")
        .setRequired(true)
        .addChoices(
  ...
  async execute(interaction) {
    ...
    if (!target.moderatable) {
      return interaction.reply({
        embeds: [{ color: 0xff4444, title: "❌ Cannot Mute", description: "I can't mute this member — they may have a higher role than me." }],
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      const expiresAt = Math.floor((Date.now() + seconds * 1000) / 1000);
